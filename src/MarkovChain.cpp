...
state_single MarkovChain::generateObservation(const state_sequence& prevState, int maxOrderWanted, bool needChoice)
{
    ...
    if (maxOrderWanted > MarkovChain::maxOrder) maxOrderWanted = MarkovChain::maxOrder;
...
// Method to modify the order of the chain.
//
// This method modifies the value of the `maxOrder` member variable.
// The `maxOrder` member variable determines the maximum length of the Markov chain.
//
// **NOTE:** This method has not been tested, so it is not guaranteed to work.
void MarkovChain::setMaxOrder(unsigned long newMaxOrder) {
    MarkovChain::maxOrder = newMaxOrder;
}